...
export async function seedRawSourceUploads(prisma: PrismaClient, tenantId: string): Promise<string[]> {
  const makeCuid = () =>
  ...
  for (const u of seed) {
    const existing = (await prisma.$queryRaw<
      Array<{ id: string }>
    >`SELECT id FROM raw_source_uploads WHERE s3_image_url = ${u.s3ImageUrl} AND extracted_text = ${u.extractedText} AND tenant_id = ${tenantId} LIMIT 1`)?.[0];
    if (existing?.id) {
