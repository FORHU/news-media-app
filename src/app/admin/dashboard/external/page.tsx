...
import { Globe2, CheckCircle, Ban, FileText, AlertCircle, RefreshCw, ExternalLink, Clock, EyeOff } from "lucide-react";
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <div className="flex items-start gap-4 p-4">
                  ...
                  <div className="flex-1 min-w-0 space-y-2">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-[10px] text-gray-400 font-medium">{article.tenant?.siteName ?? article.tenant?.domain}</span>
        ...
        {previewArticle && (
          <div className="sticky top-8 h-fit bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            {previewArticle.imageUrl && (
              <img src={previewArticle.imageUrl} alt={previewArticle.title} className="w-full h-40 object-cover" />
            )}
            <div
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between pt-1">
          <p className="text-xs text-gray-400 font-medium">{total} submission{total !== 1 ? "s" : ""}</p>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setPage((p) => Math.max(1, p - 1))}
              disabled={page === 1}
              className="px-3 py-1.5 rounded-xl text-xs font-bold text-gray-500 hover:text-gray-700 hover:bg-gray-100 disabled:opacity-30 disabled:pointer-events-none transition-colors"
            >
              Prev
            </button>
            <span className="text-xs font-black text-gray-500 min-w-[60px] text-center">{page} / {totalPages}</span>
            <button
