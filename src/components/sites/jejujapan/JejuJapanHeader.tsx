...
export default function JejuJapanHeader({ onOpenNewsletter }: HeaderProps) {
  ...
  return (
    <>
      ...
      {/* JejuJapan Sidebar Drawer — dark ink theme */}
      <AnimatePresence>
        {isSidebarOpen && (
          <>
            <motion.div
            ...
            <RemoveScroll enabled={isSidebarOpen}>
              <motion.div
                ...
                className="fixed left-0 top-0 h-[100dvh] w-full sm:w-80 max-w-[85vw] bg-[#111] z-[70] shadow-2xl flex flex-col overflow-hidden"
              >
                {/* Header */}
                <div className="flex-shrink-0 bg-[#0a0a0a] border-b-2 border-[#bc002d] h-14 px-5 flex items-center justify-between">
                  <button
                    onClick={() => setIsSidebarOpen(false)}
                ...
                <div className="flex-1 overflow-y-auto">
                  <div className="flex items-center gap-3 px-5 pt-5 pb-3">
                    <span className="h-px flex-1 bg-[#bc002d]/20" />
                    <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-[#bc002d]">Navigate</span>
                    <span className="h-px flex-1 bg-[#bc002d]/20" />
                  </div>
                  <nav>
                    {categoryLinks.map((cat) => (
                      <Link
                        ...
                        href={cat.link}
                        onClick={() => setIsSidebarOpen(false)}
                        className="group flex items-stretch border-b border-white/5 hover:bg-white/[0.03] transition-all duration-200"
                      >
                        <span className="w-[3px] shrink-0 bg-transparent group-hover:bg-[#bc002d] transition-colors" />
                        <span className="flex-1 pl-5 pr-4 py-3.5 text-[14px] font-bold text-zinc-400 group-hover:text-white transition-colors tracking-wide">
                          {cat.name}
                        </span>
                      </Link>
                    ))}
                ...
                {/* Footer */}
                <div className="flex-shrink-0 bg-[#0a0a0a] border-t border-[#bc002d]/20 p-5">
                  <button
                    type="button"
                    onClick={() => { setIsSidebarOpen(false); onOpenNewsletter?.(); }}
                    className="w-full flex items-center justify-center gap-3 py-3 border border-[#bc002d] text-[#bc002d] text-xs font-black uppercase tracking-widest hover:bg-[#bc002d] hover:text-white transition-all mb-4"
                  >
                    <Mail className="w-4 h-4" />
                    Newsletter
                  </button>
                  <div className="text-center">
                    <p className="text-[9px] text-zinc-600 font-bold uppercase tracking-[0.2em] mb-3">Follow Us</p>
                    <div className="flex items-center justify-center gap-5">
                      <a href="#" className="text-zinc-600 hover:text-[#bc002d] transition-colors">
