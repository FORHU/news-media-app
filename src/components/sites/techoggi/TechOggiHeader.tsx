...
export default function TechOggiHeader({
  domain,
  onOpenNewsletter,
  ...
  /** Live, MediaStack-classified categories (see categorize.ts); falls back
   *  to the static TECH_CATEGORIES_IT list if not supplied. */
  categories?: string[];
}) {
  ...
  const categories = categoriesProp ?? getCoreCategories(domain);
  const homeLabel = getHomeCategoryLabel(domain);
  ...
  return (
    <header
      ...
      className="sticky top-0 z-50 bg-[var(--tn-surface)] border-b border-[var(--tn-rule)]"
    >
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center gap-3 xl:gap-6 h-full">
            <button
              ...
              className="p-2 min-w-[44px] min-h-[44px] flex items-center justify-center text-[var(--tn-ink)] hover:text-[var(--tn-accent)] transition-colors rounded-full"
              aria-label="Apri il menu di navigazione"
              aria-expanded={isMenuOpen}
              aria-controls="techoggi-drawer"
            >
          ...
          <div className="flex items-center gap-2 xl:gap-5 h-full">
            ...
            {onOpenNewsletter && (
              <button
                ...
                className="hidden md:inline-flex items-center bg-[var(--tn-accent)] text-[var(--tn-accent-ink)] text-[11px] font-bold uppercase tracking-[0.16em] px-4 py-2 rounded-full hover:opacity-90 transition-opacity"
              >
                Iscriviti
              </button>
            )}
      ...
      {/* Category rail — labels come from the MediaStack classifier, so every
          link actually resolves to at least one article. */}
      <div className="border-t border-[var(--tn-rule)] hidden lg:block overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <nav className="max-w-[1440px] mx-auto px-6 py-2.5 flex items-center justify-start xl:justify-center gap-6 xl:gap-8 text-[11px] font-bold uppercase tracking-[0.14em]">
          <Link
            href="/"
          ...
          <div className="max-w-[1440px] mx-auto">
            <form onSubmit={handleSearch} className="relative max-w-2xl mx-auto">
              ...
              <input
                ...
                placeholder="Cerca notizie, argomenti…"
                className="w-full pl-12 pr-4 py-3 rounded-full border border-[var(--tn-rule)] bg-[var(--tn-accent-soft)] text-lg text-[var(--tn-ink)] placeholder:text-[var(--tn-muted)] focus:outline-none focus:border-[var(--tn-accent)] transition-colors"
                autoFocus
              />
              {showSuggestions && (
                <SearchDropdown
      ...
      >
        ...
        <div className="px-6 py-8 space-y-5">
          <Link
            href="/"
            onClick={() => setIsMenuOpen(false)}
            className={`block font-bold text-sm uppercase tracking-[0.14em] transition-colors ${
          ...
          <div className="pt-6 mt-6 border-t border-white/15 flex flex-col gap-5">
            {onOpenNewsletter && (
              <button
                ...
                className="text-left font-bold text-sm uppercase tracking-[0.14em] text-[var(--tn-accent)] hover:text-white transition-colors"
              >
                Iscriviti
              </button>
            )}
            <ContactEmailButton
              ...
              buttonClassName="block font-bold text-sm uppercase tracking-[0.14em] text-[var(--tn-accent)] hover:text-white transition-colors"
              showIcon={false}
              showLabel
              labelText="Contattaci"
              inline
            />
            <Link
              href="/admin/dashboard"
              onClick={() => setIsMenuOpen(false)}
              className="block font-bold text-sm uppercase tracking-[0.14em] text-[var(--tn-accent)] hover:text-white transition-colors sm:hidden"
            >
              Portale Admin
            </Link>
          </div>
