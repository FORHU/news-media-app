...
import { Search, Menu, Youtube, Mail, FileText, X, ChevronDown } from "lucide-react";
        ...
        <div className="max-w-7xl mx-auto px-4 h-12 flex items-center justify-between border-b border-gray-100">
          <div className="flex items-center gap-4">
            <button
              ...
              className="bg-[#ff4500] p-2 hover:bg-[#e03d00] transition-colors"
            >
        ...
        {isSidebarOpen && (
          <>
            ...
            <RemoveScroll enabled={isSidebarOpen}>
              ...
              >
                {/* Sidebar Header */}
                <div className="flex-shrink-0 bg-[#ff4500] h-14 px-5 flex items-center justify-between">
                  <button
                    onClick={() => setIsSidebarOpen(false)}
                  ...
                  <nav className="py-2">
                    {categoryLinks.map((cat) => (
                      <Link
                        ...
                        href={cat.link}
                        onClick={() => setIsSidebarOpen(false)}
                        className="block px-6 py-3.5 text-[15px] font-bold text-gray-900 hover:text-[#ff4500] hover:bg-[#ff4500]/5 border-b border-gray-100 transition-colors"
                      >
                        {cat.name}
                      </Link>
                    ))}
                    {overflowCategories.length > 0 && (
                      <details className="group">
                        <summary className="flex items-center justify-between px-6 py-3.5 text-[15px] font-bold text-gray-900 hover:text-[#ff4500] hover:bg-[#ff4500]/5 border-b border-gray-100 transition-colors cursor-pointer list-none [&::-webkit-details-marker]:hidden">
                          <span>More</span>
                          <ChevronDown className="w-4 h-4 text-gray-400 group-hover:text-[#ff4500] group-open:rotate-180 transition-transform" />
                        </summary>
                        <div className="bg-gray-50">
                          {overflowCategories.map((name) => (
                            <Link
                              ...
                              onClick={() => setIsSidebarOpen(false)}
                              className="block px-10 py-2.5 text-sm text-gray-600 hover:text-[#ff4500] hover:bg-[#ff4500]/5 transition-colors"
                            >
                              {name}
                            </Link>
                          ))}
                ...
                <div className="flex-shrink-0 border-t border-gray-100 p-5">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-3 py-3 bg-black text-white text-xs font-black uppercase tracking-widest hover:bg-[#ff4500] transition-colors mb-4"
                  >
                  ...
                  <div className="text-center">
                    ...
                    <div className="flex items-center justify-center gap-5">
                      <a href="#" className="text-gray-700 hover:text-[#ff4500] transition-colors">
