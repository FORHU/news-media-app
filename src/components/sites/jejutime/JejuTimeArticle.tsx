  ...
  if (isError || !article) {
    return (
      <div className="flex items-center justify-center py-32 px-6">
        <div className="text-center max-w-md bg-white p-8 rounded-3xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)]">
    ...
    <main className="min-h-screen bg-[#F8FAFC] text-[#2D3748] font-roboto selection:bg-blue-100 pb-20">
      ...
      <div className="bg-white border-b border-blue-100 pt-8 pb-12 shadow-sm mb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <button onClick={() => window.history.length > 1 ? router.back() : router.push("/")} className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-blue-600 mb-8 transition-colors bg-slate-50 px-4 py-2 rounded-full hover:bg-blue-50">
                <ArrowLeft className="w-4 h-4" /> Back
            </button>
            <div className="max-w-4xl mx-auto text-center">
                {normalizeCategoryName(article.category?.categoryName) && (
                    <span className="inline-block px-4 py-1.5 bg-blue-50 text-blue-700 rounded-full text-xs font-bold uppercase tracking-widest mb-6 shadow-sm border border-blue-100">
                    {normalizeCategoryName(article.category?.categoryName)}
                    </span>
                )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <article className="bg-white p-8 md:p-12 rounded-3xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] border border-slate-100">
                {youtubeId ? (
                <>
