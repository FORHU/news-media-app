...
export default function ReadRawArticleModal({
    article,
    open,
    onOpenChange,
}: ReadRawArticleModalProps) {
            ...
            >
                ...
                <div className="relative bg-gray-900 px-6 sm:px-8 py-8 sm:py-10 overflow-hidden flex-shrink-0">
                    ...
                    <div className="relative flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
                        <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center shadow-lg shadow-orange-500/20 flex-shrink-0">
                            <Newspaper className="w-6 h-6 sm:w-8 sm:h-8 text-white fill-white/20" />
                        </div>
                            ...
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                                ...
                                <div className="flex items-center gap-2 text-gray-400 text-[10px] sm:text-xs md:text-sm font-bold">
                                    ...
                                    <span className="truncate max-w-[120px] sm:max-w-[200px]">
                                        {(() => {
                                            try { return new URL(article.crawledUrl.url).hostname }
                                            catch { return 'Source' }
                                        })()}
                                    </span>
                                </div>
                ...
                <DialogFooter className="px-6 sm:px-8 py-4 sm:py-6 bg-white border-t border-gray-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 flex-shrink-0">
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <Button
                            variant="outline"
                            onClick={handleCopy}
                            className={`rounded-xl sm:rounded-2xl font-bold h-10 sm:h-12 px-4 sm:px-6 transition-all border-gray-200 ${
                                copied ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'hover:bg-gray-50'
                            }`}
                        >
                            {copied ? (
                        ...
                        <a 
                            ...
                            className="inline-flex items-center justify-center gap-2 px-4 sm:px-6 h-10 sm:h-12 rounded-xl sm:rounded-2xl font-bold text-xs sm:text-sm text-blue-600 bg-blue-50 border border-blue-100 hover:bg-blue-100 transition-all shadow-sm group"
                        >
