...
export default function AdminSidebar({
  ...
  const menuItems = [
    ...
    },
    {
      ...
      icon: FileText,
      href: "/admin/dashboard/generated",
    },
    {
      id: "external",
      label: "External Submissions",
      icon: Globe2,
      href: "/admin/dashboard/external",
    },
    {
      id: "social-publishing",
      label: "Social Publishing",
      icon: Share2,
      href: "/admin/dashboard/social-publishing",
    },
    {
      id: "crawl",
      ...
      subSections: [
        ...
        {
          label: "X Monitoring",
          items: [{ label: "X Feed Sources", href: "/admin/dashboard/x/urls" }],
        },
        {
          label: "YouTube Conversion",
          items: [{ label: "Transcribe", href: "/admin/dashboard/youtube" }],
        },
      ],
    },
    {
  ...
  return (
    <aside
      className={`bg-gradient-to-b from-black via-gray-900 to-black text-white flex-shrink-0 flex flex-col fixed left-0 top-0 bottom-0 shadow-2xl z-50 transition-all duration-300 h-full ${
        sidebarOpen
          ? "w-64 translate-x-0"
    ...
    >
      ...
      {/* Logo */}
      <div
        className={`p-6 border-b border-gray-800 flex-shrink-0 bg-gradient-to-r from-[#ff4500] to-[#ff6b35] transition-all duration-300 ${
          !sidebarOpen ? "px-0 h-20 flex items-center justify-center" : "px-6"
        }`}
      >
        <div
          className={`transition-all duration-300 ${!sidebarOpen ? "text-center" : ""}`}
        >
          <h1
            className={`text-2xl font-bold tracking-tight transition-all duration-300 ${
              !sidebarOpen ? "text-2xl leading-none" : ""
            }`}
          >
            {sidebarOpen ? "FORHU" : "F"}
          </h1>
          {sidebarOpen && (
            <p
              className={`text-sm text-orange-100 mt-1 font-medium transition-all duration-300 opacity-100 h-auto`}
            >
              Admin Portal
            </p>
          )}
      ...
      <nav className="flex-1 p-4 overflow-y-auto scrollbar-hide">
        {menuItems.map((item) => {
          ...
          const isActive = item.href
            ? pathname === item.href
            : item.subSections?.some((s) =>
                s.items.some((si) => pathname === si.href),
              );
          ...
          return (
            <div key={item.id} className="mb-2">
              {item.href ? (
                ...
                >
                  <item.icon
                    className={`${!sidebarOpen ? "w-6 h-6" : "w-5 h-5"} flex-shrink-0 transition-transform ${isActive && !sidebarOpen ? "scale-110" : ""}`}
                  />
                  <span
                ...
                <div className="flex flex-col">
                  ...
                  >
                    <item.icon
                      className={`${!sidebarOpen ? "w-6 h-6" : "w-5 h-5"} flex-shrink-0`}
                    />
                    <span
                      className={`font-medium whitespace-nowrap transition-all duration-300 ${
                        sidebarOpen
                  ...
                  {sidebarOpen && isExpanded && item.subSections && (
                    <div className="mt-2 ml-4 flex flex-col gap-2 border-l border-gray-700 pl-4 py-1">
                      {item.subSections.map((section, sIdx) => {
                        const isSubExpanded = expandedSubSections.includes(
                          section.label,
                        );
                        return (
      ...
      <div className="p-4 border-t border-gray-800 bg-gradient-to-b from-black to-gray-900 flex-shrink-0 mt-auto">
        ...
        <button
          ...
          className={`w-full flex items-center rounded-xl text-sm transition-all duration-300 ease-in-out font-bold shadow-md hover:shadow-lg bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white ${
            sidebarOpen
        ...
        >
          <LogOut
            className={`${!sidebarOpen ? "w-6 h-6" : "w-4 h-4"} flex-shrink-0`}
          />
          <span
