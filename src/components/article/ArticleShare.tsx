...
const BrandIcon = ({
  ...
  const iconProps = {
    className: cn("w-full h-full transition-colors", className),
  };
    ...
    case "Facebook":
      return (
        <FaFacebook
          {...iconProps}
          color="#1877F2"
          className={cn(iconProps.className, "group-hover:!text-white")}
        />
      );
    case "Messenger":
      return (
    ...
    case "KakaoTalk":
      return (
        <SiKakaotalk
          {...iconProps}
          color="#3C1E1E"
          className={cn(iconProps.className, "group-hover:!text-[#3C1E1E]")}
        />
      );
    default:
      return null;
...
export function ArticleShare({
  ...
  const shareOptions = useMemo(
    ...
    [title, shareUrl, handleKakaoShare],
  );
  ...
  const getThemeStyles = () => {
    switch (site) {
      case "voicejeju":
        return {
          trigger:
            "bg-black hover:bg-gray-900 text-white rounded-none px-8 py-2.5 transition-all font-inter text-[11px] font-black uppercase tracking-[0.2em] border border-black shadow-sm",
          modal:
            "sm:max-w-lg bg-white border-black border-2 rounded-none shadow-2xl",
          header:
            "text-black font-voltaire text-3xl font-normal uppercase tracking-tight",
          item: "rounded-none border-gray-100 hover:border-black transition-all",
          copyBtn:
            "bg-black text-white hover:bg-gray-900 rounded-none border-none",
        };
      case "jejutime":
        return {
          trigger:
            "bg-blue-600 hover:bg-blue-700 text-white rounded-full px-6 py-2 shadow-md transition-all font-roboto text-sm font-bold tracking-wide",
          ...
          copyBtn:
            "bg-slate-50 hover:bg-blue-50 text-blue-600 border-slate-100",
        };
      case "jejuqq":
        return {
          trigger:
            "bg-[#dc2626] hover:bg-[#b91c1c] text-white rounded-none px-8 py-3 transition-colors font-garamond text-lg font-black uppercase tracking-widest border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]",
          modal:
            "sm:max-w-lg bg-[#fdf2f2] border-2 border-black rounded-none shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]",
          header:
            "text-black font-garamond text-3xl font-black uppercase tracking-tight",
          item: "rounded-none border-2 border-black hover:bg-white transition-all",
          copyBtn:
            "bg-white hover:bg-black hover:text-white text-black border-2 border-black rounded-none",
        };
      case "jejujapan":
        return {
          trigger:
            "bg-black hover:bg-[#bc002d] text-white rounded-none px-8 py-2 transition-colors font-noto text-sm font-black uppercase tracking-[0.2em]",
          modal: "sm:max-w-lg bg-white border-4 border-black rounded-none",
          header:
            "text-black font-noto text-2xl font-black uppercase tracking-widest",
          item: "rounded-none border-black hover:border-[#bc002d] transition-all",
          copyBtn:
            "bg-black text-white hover:bg-[#bc002d] rounded-none border-none",
        };
      case "skyblueprime":
        return {
          trigger:
            "bg-sky-950 hover:bg-sky-800 text-white rounded-none px-10 py-3 transition-colors font-sans text-xs font-black uppercase tracking-widest shadow-md",
          modal:
            "sm:max-w-lg bg-white border-[3px] border-sky-950 rounded-none shadow-2xl",
          ...
          copyBtn:
            "bg-sky-950 text-white hover:bg-sky-800 rounded-none border-none",
        };
      case "lavaguetech":
        return {
          trigger:
            "bg-red-600 hover:bg-red-500 active:bg-red-700 text-white px-8 py-2.5 text-[10px] font-black uppercase tracking-widest transition-colors rounded-none border border-red-700",
          modal:
            "sm:max-w-lg bg-white border-2 border-teal-700 rounded-none shadow-2xl",
      ...
      default:
        return {
          trigger:
            "bg-[#ff4500] hover:bg-[#e03d00] text-white rounded-xl px-6 py-2 shadow-sm transition-colors font-sans text-sm font-semibold",
  ...
  if (!mounted) {
    return (
      <div
        className={cn(
          "mt-12 pt-8 border-t border-slate-100 flex flex-col items-center gap-6",
          className,
        )}
      >
        <p
    ...
    >
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        ...
        <DialogContent className={cn(styles.modal, "max-w-[95vw] sm:max-w-lg")}>
          <DialogHeader>
            <DialogTitle className={styles.header}>Share this</DialogTitle>
          </DialogHeader>
          ...
          <div className="mt-6 w-full overflow-hidden">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2 px-1 font-inter">
              Or copy link
            </p>
